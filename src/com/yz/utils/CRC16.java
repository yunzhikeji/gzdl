...
public class CRC16 {  
    ...
    public static int calcCrc16(byte[] data, int offset, int len) {  
        return calcCrc16withBigEndian(data, offset, len, 0xffff);  
    }  
     ...
     * @return CRC16校验值 
     */  
    public static int calcCrc16withBigEndian(byte[] data, int offset, int len, int preval) {  
        ...
        return ((ucCRCHi & 0x00ff) & 0xffff) | ((ucCRCLo & 0x00ff) << 8);  
    }  
  
    public static int calcCrc16withSmallEndian(byte[] data, int offset, int len, int preval) {  
        int ucCRCHi = (preval & 0xff00) >> 8;  
        int ucCRCLo = preval & 0x00ff;  
        int iIndex;  
        for (int i = 0; i < len; ++i) {  
    ...
    public static void main(String[] args) throws IOException {  
		...
		byte[] number = new byte[]{0x30,0x31,0x31};
		
		String temp = DataConvertor.toString(number);
		
		System.out.println(Integer.valueOf(temp)); 
		
		byte[] b_dust  = new byte[]{0x03,(byte)0xe8};
		
		int dust = ((b_dust[0]&0xff)<<8) + (b_dust[1]&0xff);
		System.out.println(dust); 
		
		float airlevel = (float) (((b_dust[0]&0xff)<<8)/(10.0));
		
		System.out.println(airlevel); 
        ...
        // 0x02 05 00 03 FF 00 , crc16=7C 09  a00a000103085ff81e12e35f
        int crc = CRC16.calcCrc16(new byte[] {(byte) 0xA0, (byte)0x0A, 0x00 ,0x01, 0x03 ,0x08, 0x5F ,(byte)0xF8,(byte) 0x1E,0x12});  
