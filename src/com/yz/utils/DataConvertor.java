...
public class DataConvertor {
	...
	public static long toLong(byte[] data, boolean bLittleEndian) {
		...
		if (bLittleEndian) {
			switch (iLen) {
			case 4:
				lValue = (data[0] & 0x000000ff) | ((data[1] & 0x000000ff) << 8) | ((data[2] & 0x000000ff) << 16)
			...
			case 3:
				...
				break;
			case 2:
				lValue = (data[0] & 0x000000ff) | ((data[1] & 0x000000ff) << 8);
			...
			switch (iLen) {
			case 4:
				lValue = (data[3] & 0x000000ff) | ((data[2] & 0x000000ff) << 8) | ((data[1] & 0x000000ff) << 16)
						| ((data[0] & 0x000000ff) << 24);
				break;
			case 3:
				lValue = (data[2] & 0x000000ff) | ((data[1] & 0x000000ff) << 8) | ((data[0] & 0x000000ff) << 16);

				break;
			case 2:
				lValue = (data[1] & 0x000000ff) | ((data[0] & 0x000000ff) << 8);
	...
	public static String stringTolatitude(String src) {
		int degrees;
		float minutes;

		degrees = Integer.parseInt(src.substring(0, 2));
		if (src.substring(2, 3).equals("0")) {
			minutes = Float.parseFloat(src.substring(3));
			
		} else {
			minutes = Float.parseFloat(src.substring(2));
...
//		System.out.println("the minutes is"+minutes);
	...
	public static String stringTolongitude(String src) {
		int degrees;
		float minutes;

		degrees = Integer.parseInt(src.substring(0, 3));
		if (src.substring(3, 4).equals("0")) {
			minutes = Float.parseFloat(src.substring(4));
